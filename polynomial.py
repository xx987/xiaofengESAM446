...
# addition, subtraction, multiplication, and equality.   
    ...
    def equal(self, P2):
        order1,coefs1 = self.order, self.coefs
        order2,coefs2 = P2.order, P2.coefs
        if order1 == order2 and coefs1 == coefs2:
            a = True
        else:
            a = False
        ...
        for num in range(len(coefs1)):
            ...
            if morder >= orderlist2[-1]:
                ...
                if coefs2[-1] !=0:
                    ...
                    c1 = Polynomial(newcoef).mut(firp)
                    #print(c1.coefs)
                    c2 = Polynomial(coefs1).subtract(c1).coefs#coeff of reminder term
                    co2 = Polynomial(coefs1).subtract(c1).order
...
class RationalPolynomial:
        ...
        def add(self, other):
            ...
            hafnumo = othnu.mut(selfde) 
            hafnumt = selfnu.mut(othde)
            numerator = hafnumo.add(hafnumt)
            denominator = selfde.mut(othde)
            return RationalPolynomial(numerator, denominator)
        ...
        def equal(self, other):
            ...
            if selfnu == othnu and selfde == othde:
                a = True
            else:
                a = False
            return a
        ...
        def div(self, other):
            ...
            selfde = self.denominator
            othde =  other.denominator
            numerator = selfnu.mut(othde)
            denominator = selfde.mut(othernu)
            return RationalPolynomial(numerator, denominator)
