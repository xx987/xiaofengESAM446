...
class BackwardDifferentiationFormula(Timestepper):
    ...
    def _step(self, dt):
        ...
        for i in range(lengu):
            if i>1:
                ...
                self.coep.append(i)
        steps = min(self.steps, len(self.firarr))
        coe = BackwardDifferentiationFormula._coe(tuple(self.firarr[-steps:]))
        fina = spla.spsolve(self.func.matrix - coe[-1] * sparse.eye(lengu, lengu),np.stack(self.secarr[-steps:], axis=1)@ coe[:-1])
        return fina
    ...
    def _coe(firarr):
        ...
        check = []
        steps = len(firarr)
        x = np.cumsum(np.array((0,) + firarr))
        coe = np.zeros((steps + 1,))
        ...
        for i in range(steps + 1):
            ...
            for j in range(steps + 1):
                if i ==j:
                    newlist.append(i)
                if i != j:
                    polyno = np.convolve(polyno, np.array([1, -x[j]]))
                    polyno /= x[i] - x[j]
            polyno = polyno[:-1] * np.arange(steps, 0, -1)
            coe[i] = polyno @ (x[-1] ** np.arange(steps - 1, -1, -1))
        return coe
